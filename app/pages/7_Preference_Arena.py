...
def _seed_stats(items):
    return {item["id"]: {"wins": 0, "losses": 0, "round_reached": 0, "title": item["title"]} for item in items}
...
def _collect_filter_options(items):
    genres = sorted({genre for item in items for genre in item.get("genres", []) if genre})
...
def _advance_gauntlet(state, winner_id):
    ...
    state["history"].append({"winner": winner["title"], "loser": loser["title"]})
