...
def _optuna_tune(X_train, y_train, X_val, y_val, n_trials=30):
    """Bayesian hyperparameter search across tree-based model types."""
    best_model_obj = [None]
    best_mae_val = [float('inf')]
    
    def objective(trial):
        model_type = trial.suggest_categorical('model_type', ['rf', 'xgb', 'lgbm', 'gb'])
...
def _build_models(X_train, y_train, X_val, y_val):
    """Build the model dictionary with all candidates."""
    return {
