...
def prettify_model_name(name):
    mapping = {
        "Baseline (Hist Mean)": "Baseline",
        "Baseline (Hist Mean) (Test Eval)": "Baseline",
        "Ridge Regression": "Ridge Regression",
        "Ridge Regression (Test Eval)": "Ridge Regression",
        "Bayesian Ridge": "Bayesian Ridge",
        "Bayesian Ridge (Test Eval)": "Bayesian Ridge",
        "Decision Tree (Raw)": "Decision Tree",
        "Decision Tree (Raw) (Test Eval)": "Decision Tree",
        "Decision Tree (Pruned)": "Decision Tree (Pruned)",
        "Decision Tree (Pruned) (Test Eval)": "Decision Tree (Pruned)",
        "Random Forest": "Random Forest",
        "Random Forest (Test Eval)": "Random Forest",
    }
...
st.title("🎬 AniList Score Predictor - Versione Base")
st.markdown("A simplified academic demo focused on the core profile and recommendation workflow.")
...
if mode == "AniList Profile (Machine Learning)":
    ...
    if st.session_state[model_trained_key]:
        ...
        with col1:
            st.metric("Model", prettify_model_name(model_artifact['model_name'])[:25])
        with col2:
        ...
        with st.expander("📋 Model Details & Comparison"):
            metrics_df = pd.DataFrame(model_artifact['metrics']).copy()
            if "model" in metrics_df.columns:
                metrics_df["model"] = metrics_df["model"].apply(prettify_model_name)
            ...
            fig.update_layout(xaxis_tickangle=-30, height=400, template='plotly_dark')
        ...
        with col_f1:
            genres_opts = ["None", "Action", "Adventure", "Comedy", "Drama", "Fantasy", "Horror", "Mahou Shoujo", "Mecha", "Music", "Mystery", "Psychological", "Romance", "Sci-Fi", "Slice of Life", "Sports", "Supernatural", "Thriller"]
