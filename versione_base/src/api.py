...
def _has_blocked_tag(tags):
    for tag in tags or []:
        name = str(tag.get("name") or "").strip().lower()
        if any(term in name for term in BLOCKED_TAG_TERMS):
            return True
    return False


def _is_safe_media(media):
    if not media:
        return False
    if media.get("isAdult"):
        return False
    genres = {str(g).strip() for g in media.get("genres", []) if str(g).strip()}
    if genres.intersection(BLOCKED_GENRES):
        return False
    if _has_blocked_tag(media.get("tags", [])):
        return False
    return True


def _filter_safe_entries(list_groups):
    safe_groups = []
    for lst in list_groups:
        safe_entries = [entry for entry in lst.get("entries", []) if _is_safe_media(entry.get("media", {}))]
        if safe_entries:
            new_list = dict(lst)
            new_list["entries"] = safe_entries
            safe_groups.append(new_list)
    return safe_groups
...
def fetch_user_anime_list(username: str, force_refresh: bool = False):
    ...
    unified_data = _filter_safe_entries(list(all_lists.values()))
...
def search_anime_by_title(title: str):
    ...
    return [media for media in results if _is_safe_media(media)]
